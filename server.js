...
MongoClient.connect('mongodb://localhost:27017/MarketInventoryDetails', (err,database) =>{        
	if(err) return console.log(err)
	db=database.db('MarketInventoryDetails')     
	app.listen(5000,() =>{
...
app.post('/update', (req,res)=>{			
		db.collection('Stocks').find().toArray((err,result)=>{	   			
			...
			db.collection('Stocks').findOneAndUpdate({pid:req.body.id},{
				$set:{stock: parseInt(s) +parseInt(req.body.stock)}} , {sort:{_id:-1}},
				(err,result) =>{
					if(err)
						return console.log(err)
					console.log(req.body.id+'stock changes updated')
					res.redirect('/')
				})
			})	
		})
...
app.post('/delete',(req,res)=>{
	db.collection('Stocks').findOneAndDelete({pid:req.body.id}, (err,result)=>{    
		if(err)
			return console.log(err)
		res.redirect('/')
	})
